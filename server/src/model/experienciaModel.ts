...
class Experiencia {
    ...
    async editExperiencia(id_exp: any, body: iExperiencia) {
        ...
        const { nombre, img, descripcion, precio, duracion, accesibilidad, ubicacion, transporte, tiempo }: iExperiencia = body;

        const queryStr = 'UPDATE experiencia SET (nombre, img, descripcion, precio, duracion, accesibilidad, ubicacion, transporte, tiempo)=($1, $2, $3, $4, $5, $6, $7, $8, $9) WHERE id_experiencia =$10  returning *'
        await this.client.connect()
        const resultado = await this.client.query(queryStr, [nombre, img, descripcion, precio, duracion, accesibilidad, ubicacion, transporte, tiempo, id_exp]);
        // await this.client.end()
        return resultado.rows[0];
    }
