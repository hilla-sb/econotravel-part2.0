...
const editResController = async (req: Request, res: Response,) => {
    try {
        ...
        const resultado:iReserva = 
        await reservaModel.editReserva(id,reserva);
            res.status(200).json({ message: `La reserva ${resultado.id_reserva} ha sido editada con éxito.` })
